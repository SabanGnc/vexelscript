print("VexelHUB v3.0 Başlatılıyor...")
--[[
...
local KeyBox = Create("TextBox", { Parent = KeyBoxFrame, Size = UDim2.new(1, -20, 1, 0), Position = UDim2.new(0, 10, 0, 0), BackgroundTransparency = 1, Text = "", PlaceholderText = "Anahtarı Giriniz...", TextColor3 = Theme.Text, Font = Enum.Font.Gotham, TextSize = 14, ClearTextOnFocus = false, TextTransparency = 1 })
...
local Topbar = Create("Frame", { Parent = MainFrame, Size = UDim2.new(1, 0, 0, 45), BackgroundColor3 = Theme.Sidebar, BorderSizePixel = 0, ZIndex = 10, ClipsDescendants = true })
...
MinimizeBtn.MouseButton1Click:Connect(function()
    ...
    if Minimized then
        ...
        if FrostyBlur and FrostyBlur.Parent then Tween(FrostyBlur, {Size = 0}, 0.5) end
    else
        ...
        if FrostyBlur and FrostyBlur.Parent then Tween(FrostyBlur, {Size = 20}, 0.5) end
    end
end)
...
Library:AddButton(PagePlayer, "Spawn Platform", function()
    local char = LocalPlayer.Character
...
Library:AddKeybind(PageSet, "Toggle UI Visibility", Enum.KeyCode.F1, nil, function()
    ...
    if MainFrame.Visible then
        if FrostyBlur and FrostyBlur.Parent then Tween(FrostyBlur, {Size = Minimized and 0 or 20}, 0.5) end
    else
        if FrostyBlur and FrostyBlur.Parent then Tween(FrostyBlur, {Size = 0}, 0.5) end
    end
end)
...
RunService.RenderStepped:Connect(function(dt)
    ...
    -- Smart Anti-Fall Damage (Gradual Slowdown)
